  ...
  return (
    <section className="section knowledge-portal">
      <div className="shell">
        ...
        <div className="knowledge-grid">{filtered.map((item) => <article key={item.title} className={item.featured ? "featured" : ""}>{item.featured && <img src="/images/community_system_2.png" alt="An African urban community" />}<div><span>{item.type}</span><h2>{item.title}</h2><p>{item.date} · {item.read}</p><button disabled>Preview <ArrowRight size={15} /></button></div></article>)}</div>
